# Stealth – File Upload Filter Bypass → AV Evasion → Log Deletion → Scheduled Task Binary Hijack → Administrator
...
**Various PHP functions** — I tried `system()`, `passthru()`, `exec()`, `shell_exec()` inside the double extension file. All returned blank pages, confirming PHP was executing but output was being suppressed or the functions were restricted.

### What Worked

I confirmed code execution by uploading a basic `.ps1` that connected back to a netcat listener on port 4444.

---

## Getting a Reverse Shell — AV Evasion

Standard PowerShell reverse shell payloads were caught by Windows Defender. I tried several evasion techniques before finding one that worked.
...
Running as `evader` I had very limited privileges — only `SeChangeNotifyPrivilege`. I attempted several privesc vectors that did not work:
...
I generated a malicious executable on Kali using msfvenom, saved it with a `.ps1` extension to bypass the upload filter, uploaded it to the target, then renamed and moved it to replace the original executable:
...
This executed the malicious binary as Administrator, giving me a reverse shell on port 6666 as Administrator. The root flag was found on the Administrator's Desktop.
